...
byte anode[2];//byte to write to the anode shift register, 8 of them, shifting the ON level in each byte in the array
...
byte red0[2], red1[2], red2[2], red3[2];
byte blue0[2], blue1[2], blue2[2], blue3[2];
byte green0[2], green1[2], green2[2], green3[2];
...
//
//
//  Serial.println("red 0bit is ");
//  Serial.println(bitRead(red, 0));
//
//  Serial.println("red 1bit is ");
//  Serial.println(bitRead(red, 1));
//
//  Serial.println("red 2bit is ");
//  Serial.println(bitRead(red, 2));
//
//  Serial.println("red 3bit is ");
//  Serial.println(bitRead(red, 3));
  ...
  int whichbyte = int(((level*4)+(row*2)+column)/4);
...
// This next variable is the same thing as before, but here we don't divide by 8, so we get the LED number 0-511
  int wholebyte=(level*4)+(row*2)+column;
//This will all make sense in a sec
 ...
 //This is 4 bit color resolution, so each color contains x4 64 byte arrays, explanation below:
  bitWrite(red0[whichbyte], wholebyte-(4*whichbyte), bitRead(red, 0));
  bitWrite(red1[whichbyte], wholebyte-(4*whichbyte), bitRead(red, 1));
  bitWrite(red2[whichbyte], wholebyte-(4*whichbyte), bitRead(red, 2)); 
  bitWrite(red3[whichbyte], wholebyte-(4*whichbyte), bitRead(red, 3)); 


  Serial.println("red0[0] is ");
  Serial.println(red0[0], BIN);

  Serial.println("red1[0] is ");
  Serial.println(red1[0], BIN);

  Serial.println("red2[0] is ");
  Serial.println(red2[0], BIN);

  Serial.println("red3[0] is ");
  Serial.println(red3[0], BIN);

  Serial.println("red0[1] is ");
  Serial.println(red0[1], BIN);

  Serial.println("red1[1] is ");
  Serial.println(red1[1], BIN);

  Serial.println("red2[1] is ");
  Serial.println(red2[1], BIN);

  Serial.println("red3[1] is ");
  Serial.println(red3[1], BIN);


  
  

  bitWrite(green0[whichbyte], wholebyte-(4*whichbyte), bitRead(green, 0));
  bitWrite(green1[whichbyte], wholebyte-(4*whichbyte), bitRead(green, 1));
  bitWrite(green2[whichbyte], wholebyte-(4*whichbyte), bitRead(green, 2)); 
  bitWrite(green3[whichbyte], wholebyte-(4*whichbyte), bitRead(green, 3));

  bitWrite(blue0[whichbyte], wholebyte-(4*whichbyte), bitRead(blue, 0));
  bitWrite(blue1[whichbyte], wholebyte-(4*whichbyte), bitRead(blue, 1));
  bitWrite(blue2[whichbyte], wholebyte-(4*whichbyte), bitRead(blue, 2)); 
  bitWrite(blue3[whichbyte], wholebyte-(4*whichbyte), bitRead(blue, 3));
...
case 0:
 for(shift_out=level; shift_out<level+1; shift_out++)
...
level = level+1;//increment the level variable by 8, which is used to shift out data, since the next level woudl be the next 8 bytes in the arrays

if(anodelevel==2)//go back to 0 if max is reached
anodelevel=0;
if(level==2)//if you hit 64 on level, this means you just sent out all 63 bytes, so go back
level=0;
...
void rainVersionThree(){//****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo****rainVersionTwo
  int x[64], y[64], z[64], addr, leds=4, bright=1, ledcolor, colowheel;
  int xx[64], yy[64], zz[64], xold[64], yold[64], zold[64], slowdown;
